...
/**
 * Create a class with an id of Transition. This class is responsible for the transition effects
 */
public class Transition {
    ...
    // Precomputed X offset between the center points of {@link #mSrcRect} and {@link #mSrcRect}.
    private float mCenterXDiff;

    //Precomputed Y offset between the center points of {@link #mSrcRect} and {@link #mSrcRect}.
    ...
    public Transition(RectF srcRect, RectF dstRect, long duration, Interpolator interpolator) {
        if (!MathUtils.haveSameAspectRatio(srcRect, dstRect)) {
            throw new com.sfiscusdevelopment.sfiscus.booklit.splashScreen.IncompatibleRatioException();
        }
        mSrcRect = srcRect;
        mDstRect = dstRect;
        mDuration = duration;
